...
$('.oe_website_sale').each(function () {
    ...
    $(oe_website_sale).on('change', 'input.js_variant_change, select.js_variant_change', function (ev) {
        ...
        var $currency = $ul.data("currency");
        var attribute_ids = $ul.data("attributes");
        var attribute_combs = $ul.data("attribute_combinations");
        var variant_ids = $ul.data("attribute_value_ids");
        var a_str = this.name.split('-').map(Number);
        var attribute_id = a_str[2];
        var selected_val = this.value;


        // find related attributes
        var related_attributes = [];
        for (var k in attribute_ids) {
            if(attribute_ids[k][2] == attribute_id) {
                related_attributes.push(+attribute_ids[k][0]);
            }
        }

        // construct related attributes values
        var select_data = {};
        _.each(attribute_combs, function(e, i, l) {
            if(e[attribute_id] && e[attribute_id][0] == selected_val) {
                var rpos = null;
                _.each(e, function(value, key, list) {
                    rpos = $.inArray(+key, related_attributes);
                    if ( rpos >= 0) {
                        if(!select_data[key]) {
                            select_data[key] = {};
                        }
                        select_data[key][value[0]] = [value[1], value[2]];
                    }
                });
            }
        });
        ...
        // refresh attributes
        if (!_.isEmpty(select_data)) {
            _.each(select_data, function(val,key,l) {
                var $sel = $parent.find(".js_variant_change[name='attribute-" + a_str[1] + "-" + key + "']");
                if ($sel.is('select')) {
                    var options = '';
                    _.each(val, function (v, k, l) {
                        ...
                        if (v[1] > 0) {
                            if ($currency[1] == 'before')
                                sb = "<span> +<span style='white-space: nowrap;'>" + $currency[0] + " <span class='oe_currency_value'>" + price_to_str(v[1]) + "</span></span></span>";
                            else
                                sb = "<span> +<span style='white-space: nowrap;'>" + "<span class='oe_currency_value'>" + price_to_str(v[1]) + "</span> " + $currency[0] + "</span></span>";

                        }
                        options += "<option value='" + k + "'><span>" + v[0] + "</span>" + sb + "</option>";
                    });
                    $sel.html(options);
                } else if($sel.is("input") && $sel.attr('type') == 'radio') {
                    var $radio_ul = $sel.closest("ul");
                    var ul_lists = '';
                    var inc = 0;
                    _.each(val, function (v, k, l) {
                        var sb = '';
                        if (v[1] > 0) {
                            if ($currency[1] == 'before')
                                sb = "<span class='badge'> +<span style='white-space: nowrap;'>" + $currency[0] + " <span class='oe_currency_value'>" + price_to_str(v[1]) + "</span></span></span>";
                            else
                                sb = "<span class='badge'> +<span style='white-space: nowrap;'>" + "<span class='oe_currency_value'>" + price_to_str(v[1]) + "</span> " + $currency[0] + "</span></span>";

                        }
                        var chk = inc>0?'':'checked="checked"';
                        var li = "<li class='form-group js_attribute_value' style='margin: 0;'>" +
                            "<label class='control-label' style='margin: 0 20px;'>" +
                            "<input type='radio' class='js_variant_change' " + chk + "' name='attribute-" + a_str[1] + "-" + key + "' value='" + k + "' style='vertical-align: top; margin-right: 10px;'/>" +
                            "<span>" + v[0] + "</span> " + sb + "</label></li>";

                        ul_lists += li;
                        inc += 1;
                    });
                    $radio_ul.html(ul_lists);
                }
            });
        } else {
            _.each(related_attributes, function(e,i,l) {
                var $sel = $parent.find(".js_variant_change[name='attribute-" + a_str[1] + "-" + e + "']");
                $sel.html('');
            });
        }
